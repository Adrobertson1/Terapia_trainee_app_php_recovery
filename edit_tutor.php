    ...
    try {
      if ($new_password) {
        if (!isValidPassword($new_password)) {
          ...
          $stmt = $pdo->prepare("UPDATE tutors SET first_name=?, surname=?, email=?, password=?, role=?, job_title=?, start_date=?, telephone=?, profile_image=? WHERE tutor_id=?");
          $stmt->execute([$first, $surname, $email, $hashed, $role, $job_title, $start_date, $telephone, $profile_image, $tutor_id]);
          $success = "Tutor updated successfully.";
        }
      } else {
        $stmt = $pdo->prepare("UPDATE tutors SET first_name=?, surname=?, email=?, role=?, job_title=?, start_date=?, telephone=?, profile_image=? WHERE tutor_id=?");
        $stmt->execute([$first, $surname, $email, $role, $job_title, $start_date, $telephone, $profile_image, $tutor_id]);
        $success = "Tutor updated successfully.";
      }
